...
export default function Home() {
  ...
  function handleUploadSuccess(assessmentId: string, company: string) {
    // Set the new assessment as active immediately after upload
    setAssessment({ id: assessmentId, company, industry: "", scope: "" });
    setMessages([{
      role: "assistant",
      content: `✅ Assessment uploaded for **${company}**.\n\nYour risks are ready. Ask me anything about this assessment.`,
      timestamp: new Date(),
    }]);
    setView("chat");
  }
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {assessment ? (
              <p className="text-xs text-gray-500">{assessment.company}</p>
            ) : (
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Toggle between upload and chat */}
          <button
            onClick={() => setView(view === "upload" ? "chat" : "upload")}
            className="text-xs px-3 py-1.5 border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-600"
          >
            {view === "upload" ? "← Back to chat" : "Upload CSV"}
          </button>
          <button
            onClick={handleSeed}
            disabled={seeding}
              ...
              {isEmpty ? (
                <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-center">
                  ...
                  <p className="text-gray-500 text-sm mb-6 max-w-md">
                    Upload your risk assessment CSV or load the demo data to get started.
                  </p>
                  {!assessment ? (
                    <div className="flex gap-3">
                      ...
                      <button
                        onClick={handleSeed}
                        disabled={seeding}
                        className="px-4 py-2 border border-gray-200 text-gray-700 rounded-lg text-sm hover:bg-gray-50 disabled:opacity-50"
                      >
                        {seeding ? "Loading..." : "Load demo data"}
                      </button>
                    </div>
            ...
            <div className="max-w-3xl mx-auto">
              <div className="flex gap-3 items-end">
                <textarea
                  ...
                  placeholder={assessment ? "Ask about your risks..." : "Load an assessment first"}
                  value={input}
